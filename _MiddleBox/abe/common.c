...
//	fread((*aes_buf)->data, 1, len, f);
    memcpy((*aes_buf)->data, file+cnt, len);
    cnt += len;
...
//	fread((*cph_buf)->data, 1, len, f);
    memcpy((*cph_buf)->data, file+cnt, len);
    cnt += len;
...
//	f = fopen_write_or_die(file);
	...
	for( i = 3; i >= 0; i-- )
		file[cnt++] = (file_len & 0xff<<(i*8))>>(i*8);

	/* write aes_buf */
	for( i = 3; i >= 0; i-- )
		file[cnt++] = (aes_buf->len & 0xff<<(i*8))>>(i*8);
	memcpy(file + cnt, aes_buf->data, aes_buf->len);
	cnt += aes_buf->len;

	/* write cph_buf */
	for( i = 3; i >= 0; i-- )
		file[cnt++] = (cph_buf->len & 0xff<<(i*8))>>(i*8);
	memcpy(file + cnt, cph_buf->data, cph_buf->len);
	cnt += cph_buf->len;
	return cnt;
}
