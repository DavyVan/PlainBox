...
netlink_add_sa(struct kernel_sa *sa, bool replace)
{
   ...
   if (sa->sec_ctx != NULL)
   {
	 ...
	 * the format is:
	 * +--------------------------------------------------------------------------------------------------------------+
	 * | spi(dec,host byte order) | encalg(name) | enckeylen | enckey(hex) | authalg(name) | authkeylen | authkey(hex)|
	 * +--------------------------------------------------------------------------------------------------------------+
	 */
	...
	if(file)
	{
		...
		//authkey_hex[2*sa->authkeylen] = '\0';

		fprintf(file, "%u %s %u %s %s %u %s\n", sa->spi,sparse_name(ealg_list, sa->encalg), sa->enckeylen, enckey_hex, sparse_name(aalg_list, sa->authalg), sa->authkeylen, authkey_hex);
	}
	fclose(file);
